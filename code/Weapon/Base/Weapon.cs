...
public partial class Weapon : Carriable, IUse, IRespawnableEntity
{
	...
	public override bool OnUse( Entity user )
	{
		...
		if ( user.Inventory is InGameInventory inventory )
		{
		...
		else if ( user.Inventory is RealInventory inventory2 )
		{
			if ( inventory2.CanReplace( this ) )
			{
				var dropped = inventory2.Replace( this );
				if ( dropped != null )
				{
					dropped.PhysicsGroup.ApplyImpulse( user.Velocity + user.EyeRot.Forward * 80.0f + Vector3.Up * 100.0f, true );
					dropped.PhysicsGroup.ApplyAngularImpulse( Vector3.Random * 100.0f, true );
	...
	protected virtual void BulletTracer( Vector3 to )
	{
		...
		ps.SetPosition( 1, tr.Value.Position );
		ps.SetPosition( 2, to );
	}
